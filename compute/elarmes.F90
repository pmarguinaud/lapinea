...
USE REDUCTION , ONLY : REDUCEK1, REDUCEP2, IREDUCTION_POW2, &
...
INTEGER(KIND=JPIM) :: ISTESB (YDGEOMETRY%YRDIM%NPROMA), ISTESB_TOT

INTEGER(KIND=JPIM) :: ISTEST (YDGEOMETRY%YRDIM%NPROMA), ISTEST_TOT
...
CALL REDUCEP2 (PB2(KST:KPROF,MSLB2VRL:MSLB2VRL+NFLEVG-1), &
           & ZNOR, IREDUCTION_MAX, IREDUCTION_POW2)
CALL REDUCEP2 (PB2(KST:KPROF,MSLB2URL:MSLB2URL+NFLEVG-1), &
...
DO JITER=1,NITMP
  ...
  CALL REDUCEK1 (ISTEST (KST:KPROF), ISTEST_TOT, IREDUCTION_SUM, IREDUCTION_NONE)

  IF (ISTEST_TOT > 0 .AND. JITER == NITMP) THEN
  ...
  CALL REDUCEK1 (ISTESB (KST:KPROF), ISTESB_TOT, IREDUCTION_SUM, IREDUCTION_NONE)

  IF (ISTESB_TOT > 0 .AND. JITER == NITMP) THEN
  ...
  IF(JITER /= NITMP) THEN
    ...
    CALL LARCINA(YDGEOMETRY,YDML_DYN,KST,KPROF,YDSL,IHVI,KSTABUF,LFINDVSEP,LLSLHD,LLSLHDQUAD,LLINTV,&
     ...
     & PB1(:,MSLB1UR0),PB1(:,MSLB1VR0),PB1(:,MSLB1WR0),&
     & KVSEPC,KVSEPL,PCCO,&
     & PUF,PVF,ZWF,ZWFSM,&
     & KL0,KLH0,KLEV,PLSCAW,PRSCAW,IDEP)
