...
IF (LDPLANE) THEN
  ...
  CALL ELASCAW(&
   ...
   & PRSCAW(:,:,YDML_DYN%YYTRSCAW%M_WVDERW),PRSCAW(:,:,YDML_DYN%YYTRSCAW%M_WHVW),KDEP)
...
IF (LDINTV) THEN
  CALL LAITLI(YDSL%NASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
   & PLSCAW(:,:,YDML_DYN%YYTLSCAW%M_WDLAT),PLSCAW(:,:,YDML_DYN%YYTLSCAW%M_WDLO+1),KL0(:,:,1:2),&
   & PLSCAW(:,:,YDML_DYN%YYTLSCAW%M_WDVER),PURL0,PUF0)
  CALL LAITLI(YDSL%NASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
   & PLSCAW(:,:,YDML_DYN%YYTLSCAW%M_WDLAT),PLSCAW(:,:,YDML_DYN%YYTLSCAW%M_WDLO+1),KL0(:,:,1:2),&
   & PLSCAW(:,:,YDML_DYN%YYTLSCAW%M_WDVER),PVRL0,PVF0)
  CALL LAITLI(YDSL%NASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
   & PLSCAW(:,:,YDML_DYN%YYTLSCAW%M_WDLAT),PLSCAW(:,:,YDML_DYN%YYTLSCAW%M_WDLO+1),KL0(:,:,1:2),&
   & PLSCAW(:,:,YDML_DYN%YYTLSCAW%M_WDVER),PWRL0,PWF0)
  IF(LSVTSM) THEN
    CALL ABOR1 ('UNEXPECTED CASE')
  ENDIF
ENDIF
