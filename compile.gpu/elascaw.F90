...
!DIR$ PREFERVECTOR
  ...
  DO JJ=1,KDIMK
    IF ((KSPLTHOI == 1).AND.(JJ == KDIMK)) THEN
      ...
      CALL LASCAW_CLO(KFLEV,&
       ...
       & ZCLA,PCLAMAD(:,:,:,JJ),PCLA(:,:,:,JJ), YLSTACK)

      ! * Calculation of PCLO and PCLOSLD:
      CALL LASCAW_CLO(KFLEV,&
       & KPROMB,KST,KPROF,LLT_PHYS,PDLO(:,:,1),PDLOMAD(:,:,1),&
       & ZKHTURB(:,:,JJ),ZKHTURB(:,:,JJ),&
       & ZCLO(:,:,:,1),PCLOMAD(:,:,:,1,JJ),PCLO(:,:,:,1,JJ), YLSTACK)
      CALL LASCAW_CLO(KFLEV,&
       & KPROMB,KST,KPROF,LLT_PHYS,PDLO(:,:,2),PDLOMAD(:,:,2),&
       & ZKHTURB(:,:,JJ),ZKHTURB(:,:,JJ),&
       & ZCLO(:,:,:,2),PCLOMAD(:,:,:,2,JJ),PCLO(:,:,:,2,JJ), YLSTACK)
    ...
    ELSE
      ...
      CALL LASCAW_CLO(KFLEV,&
       ...
       & PKAPPA,ZKHTURB(:,:,JJ),&
       & PCLA(:,:,:,JJ),PCLAMAD(:,:,:,JJ),PCLASLD(:,:,:,JJ), YLSTACK)
      ...
      CALL LASCAW_CLO(KFLEV,&
       & KPROMB,KST,KPROF,LLT_SLHD,PDLO(:,:,1),PDLOMAD(:,:,1),&
       & PKAPPA,ZKHTURB(:,:,JJ),&
       & PCLO(:,:,:,1,JJ),PCLOMAD(:,:,:,1,JJ),PCLOSLD(:,:,:,1,JJ), YLSTACK)
      CALL LASCAW_CLO(KFLEV,&
       & KPROMB,KST,KPROF,LLT_SLHD,PDLO(:,:,2),PDLOMAD(:,:,2),&
       & PKAPPA,ZKHTURB(:,:,JJ),&
       & PCLO(:,:,:,2,JJ),PCLOMAD(:,:,:,2,JJ),PCLOSLD(:,:,:,2,JJ), YLSTACK)
  ...
  CALL LASCAW_VINTW(LDSLHDHEAT,&
   & KPROMB,KFLEV,KST,KPROF,LLCOMADV,LLT_SLHD,KLEV,&
   & PLEV,PDVER,PDVERMAD,PSTDDISW,PKAPPA,PKAPPAT,PVETA,ZVCUICO_,ZVSLD_,ZVSLDW_,&
   & PVINTW,PVINTWMAD,PVINTWSLD,PVINTWSLT, YLSTACK)
  ...
  IF (KWIS == 105) THEN
    ...
    DO JLEV=1,KFLEV
        ...
        PVINTWS(JROF,JLEV,1)=YDVSPLIP%RFVV(4,ILEVV  ,1)+ZD2*( YDVSPLIP%RFVV(4,ILEVV  ,2) +&
         & ZD2*(YDVSPLIP%RFVV(4,ILEVV   ,3) + ZD2*YDVSPLIP%RFVV(4,ILEVV  ,4) ) )  
        PVINTWS(JROF,JLEV,2)=YDVSPLIP%RFVV(3,ILEVV+1,1)+ZD2*( YDVSPLIP%RFVV(3,ILEVV+1,2) +&
         & ZD2*( YDVSPLIP%RFVV(3,ILEVV+1,3) + ZD2*YDVSPLIP%RFVV(3,ILEVV+1,4) ) )  
        PVINTWS(JROF,JLEV,3)=YDVSPLIP%RFVV(2,ILEVV+2,1)+ZD2*( YDVSPLIP%RFVV(2,ILEVV+2,2) +&
         & ZD2*( YDVSPLIP%RFVV(2,ILEVV+2,3) + ZD2*YDVSPLIP%RFVV(2,ILEVV+2,4) ) )  
        PVINTWS(JROF,JLEV,4)=YDVSPLIP%RFVV(1,ILEVV+3,1)+ZD2*( YDVSPLIP%RFVV(1,ILEVV+3,2) +&
         & ZD2*( YDVSPLIP%RFVV(1,ILEVV+3,3) + ZD2*YDVSPLIP%RFVV(1,ILEVV+3,4) ) )  
...
!DIR$ PREFERVECTOR
  ...
  CALL LASCAW_CLO(KFLEV,&
   & KPROMB,KST,KPROF,LLT_SLHD,PDLAT,PDLAMAD,&
   & PKAPPA,ZKHTURB(:,:,1),&
   & PCLA(:,:,:,1),PCLAMAD(:,:,:,1),PCLASLD(:,:,:,1), YLSTACK)

  ! * Calculation of PCLO and PCLOSLD:
  CALL LASCAW_CLO(KFLEV,&
   & KPROMB,KST,KPROF,LLT_SLHD,PDLO(:,:,1),PDLOMAD(:,:,1),&
   & PKAPPA,ZKHTURB(:,:,1),&
   & PCLO(:,:,:,1,1),PCLOMAD(:,:,:,1,1),PCLOSLD(:,:,:,1,1), YLSTACK)
  CALL LASCAW_CLO(KFLEV,&
   & KPROMB,KST,KPROF,LLT_SLHD,PDLO(:,:,2),PDLOMAD(:,:,2),&
   & PKAPPA,ZKHTURB(:,:,1),&
   & PCLO(:,:,:,2,1),PCLOMAD(:,:,:,2,1),PCLOSLD(:,:,:,2,1), YLSTACK)
