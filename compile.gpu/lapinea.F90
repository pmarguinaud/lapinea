...
SUBROUTINE LAPINEA(&
 ...
 & KVSEPC,KVSEPL,&
 ! --- OUTPUT -----------------------------------------------------------------
 & PCCO,PUF,PVF,KL0,KLH0,PLSCAW,PRSCAW,KL0H,PLSCAWH,PRSCAWH,PSCO,PGFLT1, YDSTACK)
...
REAL(KIND=JPRB) :: ZUF0(1), ZVF0(1), ZWF0(1), ZWFSM (1)
temp (REAL(KIND=JPRB), ZPROLEVDUM, (YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG))
REAL(KIND=JPRB) :: ZASLLEVDUM (YDSL%NASLB1,YDGEOMETRY%YRDIMV%NFLSA:YDGEOMETRY%YRDIMV%NFLEN)
...
DO JGFL=1,YDML_GCONF%YGFL%NUMFLDS
  IF(YDML_GCONF%YGFL%YCOMP(JGFL)%CSLINT == 'LAIHVT      ' .OR.&
     & YDML_GCONF%YGFL%YCOMP(JGFL)%CSLINT == 'LAIHVTQM    ' .OR.&
     & YDML_GCONF%YGFL%YCOMP(JGFL)%CSLINT == 'LAIHVTQMH   ' ) THEN  
    IHVI=1
  ENDIF
ENDDO
...
IF(LRPLANE) THEN
  !$acc loop vector
  DO JROF = KST, KPROF
    ZGMDTX (JROF)=0.5_JPRB*YDGEOMETRY%YRGSGEOM(KIBL)%GM(JROF)*YDML_GCONF%YRRIP%RTDT/YDGEOMETRY%YREGEO%EDELX
    ZGMDTY (JROF)=0.5_JPRB*YDGEOMETRY%YRGSGEOM(KIBL)%GM(JROF)*YDML_GCONF%YRRIP%RTDT/YDGEOMETRY%YREGEO%EDELY
  ENDDO
ENDIF
...
IF (LRPLANE) THEN
  ...
  CALL ELARMES(YDGEOMETRY,YDML_DYN,YDML_GCONF%YRRIP,KST,KPROF,YDSL,ISTALAT,ZGMDTX,ZGMDTY,PB1,PB2,&
   & ZLSDEPI,KIBL,&
   & KVSEPC,KVSEPL,PSCO,ZLEV,PCCO,PUF,PVF,&
   & KL0,KLH0,ILEV,PLSCAW,PRSCAW, YLSTACK)  
...
CALL LARCINA(YDGEOMETRY,YDML_DYN,KST,KPROF,YDSL,IHVI,ISTALAT,YDML_DYN%YRDYN%LFINDVSEP,LSLHD,LSLHDQUAD,LLINTV,&
 & ITIP,IROT,LRPLANE,ZLSDEPI,&
 & KIBL,&
 & PSCO,ZLEV,&
 & PB2(:,YDML_DYN%YRPTRSLB2%MSLB2KAPPA :YDML_DYN%YRPTRSLB2%MSLB2KAPPA +YDGEOMETRY%YRDIMV%NFLEVG-1),  &
 & PB2(:,YDML_DYN%YRPTRSLB2%MSLB2KAPPAT:YDML_DYN%YRPTRSLB2%MSLB2KAPPAT+YDGEOMETRY%YRDIMV%NFLEVG-1),  &
 & PB2(:,YDML_DYN%YRPTRSLB2%MSLB2KAPPAM:YDML_DYN%YRPTRSLB2%MSLB2KAPPAM+YDGEOMETRY%YRDIMV%NFLEVG-1),  &
 & PB2(:,YDML_DYN%YRPTRSLB2%MSLB2KAPPAH:YDML_DYN%YRPTRSLB2%MSLB2KAPPAH+YDGEOMETRY%YRDIMV%NFLEVG-1),  &
 & PB2(:,YDML_DYN%YRPTRSLB2%MSLB2STDDISU:YDML_DYN%YRPTRSLB2%MSLB2STDDISU+YDGEOMETRY%YRDIMV%NFLEVG-1),&
 & PB2(:,YDML_DYN%YRPTRSLB2%MSLB2STDDISV:YDML_DYN%YRPTRSLB2%MSLB2STDDISV+YDGEOMETRY%YRDIMV%NFLEVG-1),&
 & PB2(:,YDML_DYN%YRPTRSLB2%MSLB2STDDISW:YDML_DYN%YRPTRSLB2%MSLB2STDDISW+YDGEOMETRY%YRDIMV%NFLEVG-1),&
 & ZASLLEVDUM, ZASLLEVDUM, ZASLLEVDUM, &
 & KVSEPC,KVSEPL,PCCO,&
 & ZPROLEVDUM, ZPROLEVDUM, ZPROLEVDUM, ZPROLEVDUM, &
 & KL0,KLH0,ILEV,PLSCAW,PRSCAW,IDEP, YLSTACK)
...
!   for "2 Omega vectorial a k" recalculation.
  IF(YDML_DYN%YRDYN%LADVF) THEN
    !$acc loop vector
    DO JROF = KST, KPROF
        PSCO(JROF,1:YDGEOMETRY%YRDIMV%NFLEVG,YDML_DYN%YYTSCO%M_SINLA)=PCCO(JROF,1:YDGEOMETRY%YRDIMV%NFLEVG,YDML_DYN%YYTCCO%M_RLON)
    ENDDO

    !$acc loop vector
    DO JROF = KST, KPROF
        PSCO(JROF,1:YDGEOMETRY%YRDIMV%NFLEVG,YDML_DYN%YYTSCO%M_COPHI)=PCCO(JROF,1:YDGEOMETRY%YRDIMV%NFLEVG,YDML_DYN%YYTCCO%M_RLAT)
    ENDDO
...
IF (LNHDYN.AND.LGWADV.AND.(.NOT.LVFE_GW)) THEN
  ...
  CALL LARCINHA(YDGEOMETRY,YDML_DYN,KST,KPROF,YDSL,IHVI,ISTALAT,ITIP,IROT,ZLSDEPI,&
   & KIBL,&
   & PSCO,ZLEV,PB2(:,YDML_DYN%YRPTRSLB2%MSLB2KAPPA:YDML_DYN%YRPTRSLB2%MSLB2KAPPA+YDGEOMETRY%YRDIMV%NFLEVG-1),PB2(:,YDML_DYN%YRPTRSLB2%MSLB2KAPPAT:YDML_DYN%YRPTRSLB2%MSLB2KAPPAT+YDGEOMETRY%YRDIMV%NFLEVG-1),&
   & PB2(:,YDML_DYN%YRPTRSLB2%MSLB2KAPPAM:YDML_DYN%YRPTRSLB2%MSLB2KAPPAM+YDGEOMETRY%YRDIMV%NFLEVG-1),PB2(:,YDML_DYN%YRPTRSLB2%MSLB2KAPPAH:YDML_DYN%YRPTRSLB2%MSLB2KAPPAH+YDGEOMETRY%YRDIMV%NFLEVG-1),&
