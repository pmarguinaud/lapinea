...
temp (INTEGER(KIND=JPIM), ISTEST0, (YDGEOMETRY%YRDIM%NPROMA))
...
DO JITER=1,NITMP
  ...
  IF(JITER /= NITMP) THEN
    ...
    CALL LARCINA(YDGEOMETRY,YDML_DYN,KST,KPROF,YDSL,IHVI,KSTABUF,LFINDVSEP,LLSLHD,LLSLHDQUAD,LLINTV,&
     ...
     & PB1(1,MSLB1UR0),PB1(1,MSLB1VR0),PB1(1,MSLB1WR0),&
     & KVSEPC,KVSEPL,PCCO,&
     & PUF,PVF,ZWF,ZWFSM,&
     & KL0,KLH0,KLEV,PLSCAW,PRSCAW,IDEP,ISTPT,KSTSZ,PSTACK)
